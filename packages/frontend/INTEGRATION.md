...
- **SOURCE 1 — Contract** ([`src/data/contractData.json`](src/data/contractData.json), Typen in [`types/sources.ts`](src/types/sources.ts) → `ContractCampaign`): spiegelt die echten `Donation`/`DonationFactory`-Contracts (Branch `contractDonation`). On-chain State — `address` (Identität), `contractOwner`, `donationGoal` (= **Summe** der Meilensteinbeträge), `totalDonations`, `donors` (`getDonors()`), `start`/`end` (Unix-Timestamps), `currentStatus` (Enum `Funding|Payout|Failed|Closed`), `currentMilestoneIndex`, `totalPayout`, `milestoneVotingDeadline`, `refundableBalance`, Validator-Adressen (`getValidators()`), Meilenstein-**Struct** (`amount` **absolut**, `approvedCount`, `rejectedCount`, `votingFinished`, `paid`; `getMilestones()`). Beträge sind die **native Coin** (Spende = `msg.value`, kein ERC-20) und stehen als gut lesbare Ganz-Coin-Zahlen im Mock (on-chain Wei; Umrechnung via `parseEther`/`formatEther` an der ethers-Grenze). **Abgeleitet** im Frontend (NICHT on-chain): „Tage übrig"/„Restzeit" aus `end`, Meilenstein-**Status** aus `paid`/`votingFinished`/`currentMilestoneIndex`+`currentStatus`, laufend/abgelaufen aus `currentStatus`+`end`, „Freigabe nötig" aus der festen **66,66 %-Mehrheit** (Vertragskonstante `neededVoteMajorityInBps` — **nicht** public, also kein Getter; das Frontend spiegelt den Literalwert). **Text bleibt off-chain:** der Vertrag speichert zwar eine on-chain `description` (Projekt) und je Meilenstein eine `description`, das Frontend liest sie aber bewusst **nicht** — alle Anzeigetexte kommen aus SOURCE 2.
...
| **Meilensteine** (Status, zugeordnete Mittel, „X/N bestätigt · Y nötig") | [`components/project/MilestoneCard.vue`](src/components/project/MilestoneCard.vue) | `project.milestones` + `goalReached` (`raised >= goal`) | `getMilestones()`-Struct: `amount` (**absolut**), `approvedCount`/`rejectedCount`, `votingFinished`, `paid`. Modell: **jeder** Meilenstein — auch der erste — wird erst nach Freigabe durch die **66,66 %-Mehrheit** der Validatoren (nicht „alle N") ausgezahlt. Der gerade abgestimmte Meilenstein ist `currentMilestoneIndex` (der als nächstes auszuzahlende, noch unbezahlt); seine Freigabe löst seine eigene Auszahlung aus; (Folgeschritt) `voteMilestone`-Tx |
