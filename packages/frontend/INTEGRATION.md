...
| **Meilensteine** (Status, zugeordnete Mittel, „X/N bestätigt · Y nötig") | [`components/project/MilestoneCard.vue`](src/components/project/MilestoneCard.vue) | `project.milestones` + `goalReached` (`raised >= goal`) | `getMilestones()`-Struct: `amount` (**absolut**), `approvedCount`/`rejectedCount`, `votingFinished`, `paid`. Modell: Meilenstein 0 wird sofort ausgezahlt; jeder weitere erst nach Freigabe des vorherigen durch **66,66 %-Mehrheit** der Validatoren (nicht „alle N"). Der gerade abgestimmte Meilenstein ist `currentMilestoneIndex - 1`; (Folgeschritt) `voteMilestone`-Tx |
